...
THREE.Math.clamp(this.healthPercent+Math.min(this.healthPercentIncrease(a),this.healthPercentIncreaseMax()),0,100));h-=this.body.rotation.y;a=this.bodyAngularSpeed*a*(0>h?-1:1);this.body.rotation.y+=Math.abs(h)<Math.abs(a)?h:a},checkIntersect:function(a,b){a.updateMatrixWorld();b.updateMatrixWorld();null===b.geometry.boundingSphere&&b.geometry.computeBoundingSphere();null===a.geometry.boundingBox&&a.geometry.computeBoundingBox();var c=a.geometry.boundingBox,d=b.geometry.boundingSphere,e=b.localToTarget(d.center.clone(),
this.parent),f=e.clone().addScaledVector(this.shuttlecock.velocity,-this.shuttlecock.lastDelta);this.parent.localToTarget(e,a);this.parent.localToTarget(f,a);var h=!1,g=f.clone().sub(e),f=-e.z/g.z;if(1>=Math.abs(f)||0>f*this.lastCheckMul)e=e.clone().addScaledVector(g,f),h=this.checkIntersectRectAndCircle(c.clone().translate(e.clone().negate()),d.radius),this.lastCheckMul=f;return h},checkIntersectRectAndCircle:function(a,b){return 0>a.max.x?0>a.max.y?a.max.x*a.max.x+a.max.y*a.max.y<b*b:0<a.min.y?
a.max.x*a.max.x+a.min.y*a.min.y<b*b:Math.abs(a.max.x)<b:0<a.min.x?0>a.max.y?a.min.x*a.min.x+a.max.y*a.max.y<b*b:0<a.min.y?a.min.x*a.min.x+a.min.y*a.min.y<b*b:Math.abs(a.min.x)<b:0>a.max.y?Math.abs(a.max.y)<b:0<a.min.y?Math.abs(a.min.y)<b:!0},getTopImpactSpeed:function(a){var b=this.shuttlecock.position.clone().setY(0),c=this.targetPosition.clone().sub(b);a=c.clone().length();b=Math.abs(b.clone().distanceTo(c));b=Math.ceil(b/100)-2;for(0>b&&(b=0);b<this.topForceTable.length&&!(this.topForceTable[b][0]>
a);b++);if(0===b)return Math.PI*this.topForceTable[b][1];a=this.diff(this.topForceTable[b-1][0],this.topForceTable[b][0],a);return Math.PI*(this.topForceTable[b-1][1]+(this.topForceTable[b][1]-this.topForceTable[b-1][1])*a)},getImpactAngleAndSpeed:function(a){if(0<=this.targetPosition.x*this.shuttlecock.position.x)return[0,0];var b=this.targetPosition.clone().sub(this.shuttlecock.position).setY(0);a=b.clone().multiplyScalar(this.shuttlecock.position.x/b.x).length();b=b.length()-a;a=this.angleAndSpeed(a,
b);return[a[1],a[0]]},angleAndSpeed:function(a,b){var c=this.forceTable,d,e,f=!1,h,g;for(h=0;h<c.length;h++){if(c[h][0]>=a&&0===h){for(g=1;g<c[h].length;g++)if(c[0][g][0]>=b)return this.calculate(c[0][g-1][0],c[0][g][0],c[0][g-1][1],c[0][g][1],c[0][g-1][2],c[0][g][2],b);return[c[0][g-1][1],c[0][g-1][2]]}if(c[h][0]>=a){var k=h-1;for(g=1;g<c[k].length;g++)if(c[k][g][0]>=b&&1===g)d=[c[k][1][1],c[k][1][2]];else if(c[k][g][0]>=b){d=this.calculate(c[k][g-1][0],c[k][g][0],c[k][g-1][1],c[k][g][1],c[k][g-
1][2],c[k][g][2],b);break}g===c[k].length&&(f=!0,d=[c[k][g-1][1],c[k][g-1][2]]);for(g=1;g<c[h].length;g++)if(c[h][g][0]>=b&&1===g)e=[c[h][1][1],c[h][1][2]];else if(c[h][g][0]>=b){e=this.calculate(c[h][g-1][0],c[h][g][0],c[h][g-1][1],c[h][g][1],c[h][g-1][2],c[h][g][2],b);break}return g===c[h].length?d:f?e:this.calculate(c[k][0],c[h][0],d[0],e[0],d[1],e[1],a)}}},calculate:function(a,b,c,d,e,f,h){a=this.diff(a,b,h);return[(d-c)*a+c,(f-e)*a+e]},diff:function(a,b,c){return(c-a)/(b-a)},forceTable:[[50,
[14,23,1.5],[18,25,1.5],[44,25,1.45],[55,30,1.45],[85,30,1.4]],[60,[42,20,1.4],[61,20,1.35],[83,25,1.35],[106,25,1.3],[130,30,1.3],[139,30,1.28]],[70,[31,20,1.4],[69,20,1.3],[96,25,1.3],[120,30,1.3],[144,30,1.25],[172,30,1.2]],[80,[41,20,1.35],[59,20,1.3],[76,20,1.25],[107,25,1.25],[127,25,1.2],[158,30,1.2],[185,35,1.2]],[90,[82,20,1.2],[117,25,1.2],[133,25,1.15],[168,30,1.15],[188,30,1.1],[220,35,1.1],[249,40,1.1],[270,40,1.05]],[100,[72,20,1.2],[107,25,1.2],[123,25,1.15],[158,30,1.15],[178,30,1.1],
[210,35,1.1],[230,35,1.05],[247,35,1],[262,35,.95],[297,40,.95],[328,45,.95],[356,50,.95],[372,50,.9],[400,55,.9]],[200,[78,30,1.1],[94,30,1.05],[111,30,1],[124,30,.95],[135,30,.9],[144,30,.85],[152,30,.8],[196,35,.8],[202,35,.75],[208,35,.7],[249,40,.7],[252,40,.65],[289,45,.65],[290,45,.6],[326,50,.6],[359,55,.6],[387,60,.6],[390,60,.55],[417,65,.55],[443,70,.55],[463,75,.55],[487,80,.55],[509,85,.55],[527,90,.55],[559,100,.55],[592,110,.55],[642,130,.55]],[300,[75,35,.9],[113,40,.9],[124,40,.85],
[135,40,.8],[142,40,.75],[149,40,.7],[185,45,.7],[189,45,.65],[223,50,.65],[226,50,.6],[259,55,.6],[287,60,.6],[290,60,.55],[317,65,.55],[343,70,.55],[387,80,.55],[426,90,.55],[428,90,.5],[462,100,.5],[493,110,.5],[521,120,.5],[547,130,.5],[568,140,.5],[588,150,.5],[608,160,.5],[632,175,.5],[634,175,.45],[658,190,.45]],[400,[48,50,.97],[56,50,.95],[72,50,.9],[101,50,.8],[118,50,.7],[149,55,.7],[159,55,.6],[161,55,.55],[190,60,.55],[243,70,.55],[287,80,.55],[327,90,.55],[359,100,.55],[362,100,.5],
[392,110,.5],[421,120,.5],[447,130,.5],[468,140,.5],[488,150,.5],[491,150,.45],[510,160,.45],[543,180,.45],[570,200,.45],[596,220,.45],[616,240,.45],[634,260,.45],[649,280,.45],[660,300,.45],[662,300,.4],[667,310,.4]],[500,[42,70,.95],[65,70,.9],[99,80,.9],[140,80,.8],[167,80,.7],[184,80,.6],[187,80,.5],[228,90,.5],[262,100,.5],[293,110,.5],[321,120,.5],[347,130,.5],[368,140,.5],[388,150,.5],[391,150,.45],[410,160,.45],[443,180,.45],[470,200,.45],[496,220,.45],[516,240,.45],[534,260,.45],[549,280,
.45],[560,300,.45],[562,300,.4],[579,330,.4],[583,340,.4],[589,360,.4],[592,370,.4],[595,380,.4],[602,400,.4]],[600,[49,110,.95],[77,110,.9],[126,110,.8],[163,110,.7],[186,110,.6],[193,110,.5],[221,120,.5],[247,130,.5],[288,150,.5],[326,170,.5],[367,200,.5],[392,220,.5],[421,250,.5],[437,270,.5],[443,280,.5],[456,300,.5],[462,300,.4],[472,320,.4],[479,330,.4],[486,350,.4],[495,380,.4],[502,400,.4]],[670,[43,150,.95],[76,150,.9],[137,150,.8],[176,150,.7],[206,150,.6],[218,150,.5],[256,170,.5],[286,
190,.5],[297,200,.5],[322,220,.5],[341,240,.5],[367,270,.5],[386,300,.5],[392,300,.4],[409,330,.4],[419,360,.4],[432,400,.4]]],topForceTable:[[432.837,12],[508.317,15],[612.811,20],[697.075,25],[767.586,30],[793.203,32],[829.48,35],[849.617,37],[860.173,38],[880.123,40],[925.701,45],[966.793,50],[1002.28,55],[1036.857,60],[1064.451,65],[1091.885,70],[1118.193,75],[1140.245,80],[1160.976,85],[1182.143,90],[1200.124,95],[1217.301,100],[1233.056,105],[1248.758,110],[1261.102,115],[1274.775,120],[1287.784,
125],[1298.189,130],[1309.88,135],[1321.354,140],[1329.665,145],[1333.054,150]],topTableStart:[0,1,2,3,5,6],onAfterImpact:function(){},onBeforeUpdate:function(){}});N.prototype=Object.assign(Object.create(THREE.Mesh.prototype),{constructor:N,inArea:function(a,b){return this.getArea(a).containsPoint(b)},getArea:function(a){return this["getArea"+a]()},getAreaAll:function(){return new THREE.Box3(new THREE.Vector3(-this.geometry.parameters.longLineSingle,-this.geometry.parameters.sidelineDouble,0),new THREE.Vector3(this.geometry.parameters.longLineSingle,
...
{constructor:y}),{number:{get:function(){return this.material.getNumber()},set:function(a){this.material.setNumber(a)}}});O.prototype=Object.assign(Object.create(THREE.Mesh.prototype),{constructor:O,init:function(){for(var a=0;a<this.frontCards.length;a++)this.frontCards[a].number=0,this.transitionCards[a].number=this.backCards[a].number=1,this.transitionCards[a].visible=!1},addCard:function(a,b,c,d){var e={},f;for(f in d)e[f]=d[f];e.number=e.number||0;var h=new THREE.Object3D;h.position.set(c.x,
1.01*this.geometry.parameters.height,0);h.rotation.set(2*Math.PI-this.geometry.parameters.planeAngle,0,0);this.add(h);var g=h.clone();g.rotation.x=this.geometry.parameters.planeAngle;this.add(g);d=g.clone();this.add(d);f=new y(new THREE.PlaneGeometry(a,b),new w(e));f.position.set(0,-c.y-b/2,0);h.add(f);this.frontCards.push(f);e.number++;c=new y(new THREE.PlaneGeometry(a,b),new w(e));c.position.copy(f.position);g.add(c);this.backCards.push(c);a=new y(new THREE.PlaneGeometry(a,b),new w(e));a.position.copy(f.position);
