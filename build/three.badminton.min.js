...
b).projectOnPlane(new THREE.Vector3(0,0,1)),f=(c=c.intersectsSphere(e))&&f||!c&&!f);f=f?this.lastWinner:this.getAnotherPlayer(this.lastWinner);c=this.getScore(f);this.setScore(f,c+1);this.lastWinner=f;this.onScoreChange()}}},onScoreChange:function(){}};R.prototype={constructor:R,init:function(){this.data=[];this.dataLength=0;this.playing=!1;this.elapsed=0},record:function(a){for(;this.dataLength+a>this.maxLength;)this.data.shift(),this.dataLength-=this.data[0].delta;for(var b={delta:a,objects:[]},
c=0;c<this.objects.length;c++){var d=this.objects[c];b.objects.push({position:d.position.clone(),quaternion:d.quaternion.clone()})}this.data.push(b);1<this.data.length&&(this.dataLength+=a)},start:function(a){0>a&&(a=this.dataLength+a);0>a&&(a=0);a>this.dataLength||(this.elapsed=a,this.playing=!0)},update:function(a){if(window.xxx)debugger;this.elapsed+=a;this.elapsed>=this.dataLength&&(this.elapsed=this.dataLength,this.playing=!1);a=null;for(var b=this.data[0],c=0,d=1;d<this.data.length&&!(a=b,b=
this.data[d],c+=this.data[d].delta,this.elapsed<=c);d++);c=1-(c-this.elapsed)/b.delta;for(d=0;d<this.objects.length;d++){var e=this.objects[d],f=a.objects[d],g=b.objects[d];THREE.Quaternion.slerp(f.quaternion,g.quaternion,e.quaternion,c);e.position.lerpVectors(f.position,g.position,c)}}};K.prototype={constructor:K,get volume(){return this._volume},set volume(a){this._volume=a;this.updateVolume()},get leftVolume(){return this._leftVolume},set leftVolume(a){this._leftVolume=a;this.updateVolume()},get rightVolume(){return this._rightVolume},
...
J;m.NumberMaterial=w;m.Shuttlecock=L;m.Robot=M;m.Court=N;m.Scoreboard=O;m.ScoreboardCard=y;m.NetGroup=P;m.Game=Q;m.Record=R;m.AudioChannel=K;m.Ranking=S;Object.defineProperty(m,"__esModule",{value:!0})});
