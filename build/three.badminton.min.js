...
h.position.y=b/2;h.position.z=-c/4;h.rotation.x=f;this.add(h);c=new THREE.Mesh(new THREE.PlaneGeometry(a,c),new THREE.MeshNormalMaterial({side:THREE.DoubleSide}));c.rotation.x=Math.PI/2;this.add(c);var g=e-2*d,n=(a-5*d)/3,l=new THREE.Object3D;l.position.y=1.01*b;this.add(l);var p=[1.5*-d-1.25*n,1.5*-d-1*n,1.5*-d-.75*n,.5*-d-.25*n,.5*+d+.25*n,1.5*+d+.75*n,1.5*+d+1*n,1.5*+d+1.25*n];for(a=0;a<p.length;a++)b=new THREE.Mesh(new THREE.TorusGeometry(15,1,16,16),new THREE.MeshNormalMaterial({side:THREE.DoubleSide})),
b.position.x=p[a],b.rotation.y=Math.PI/2,l.add(b);var k=this;(new THREE.FontLoader).load("https://ikatyang.github.io/three.badminton.js/fonts/gentilis_regular.typeface.json",function(a){function b(c,e,f,h,k){var p=new E(n*e,g*e,a);p.setText(c);p.position.x=f;p.rotation.x=h;p.visible=k;p.plane.position.y=-g*e/2-d;l.add(p);return p}k.frontCards=[k.frontCard1=b("0",1,p[1],2*Math.PI-f,!0),k.frontSmallCard1=b("0",.5,p[3],2*Math.PI-f,!0),k.frontSmallCard2=b("0",.5,p[4],2*Math.PI-f,!0),k.frontCard2=b("0",
1,p[6],2*Math.PI-f,!0)];k.backCards=[k.backCard1=b(null,1,p[1],f,!0),k.backSmallCard1=b(null,.5,p[3],f,!0),k.backSmallCard2=b(null,.5,p[4],f,!0),k.backCard2=b(null,1,p[6],f,!0)];k.animateCards=[k.animateCard1=b(null,1,p[1],0,!1),k.animateSmallCard1=b(null,.5,p[3],0,!1),k.animateSmallCard2=b(null,.5,p[4],0,!1),k.animateCard2=b(null,1,p[6],0,!1)]});this.speed=2*Math.PI;this.actions=[null,null,null,null]}function L(a,b){THREE.Object3D.call(this);this.parameters={radius:a,tube:b};var c=new THREE.Mesh(new THREE.TorusGeometry(a,
...
c=this.shuttle.velocity;d=this.shuttle.position;l=f.clone().sub(d).dot(l)/c.clone().dot(l);g=c.clone().multiplyScalar(l).add(d);c=this.topLink;d=this.topRacket;f=Math.PI/2-g.clone().sub(f).angleTo(new THREE.Vector3(0,1,0));e=this.smashSpeed;b=-1E4<l?g:null;h=g.y;g=this.parameters.racketLength/2;n=new THREE.Vector3(0,this.parameters.bodyHeight+this.parameters.racketLength/2*Math.cos(f),this.parameters.racketLength/2*Math.sin(f));"common"===this.eyeStatus&&(this.body.remove(this.eyes),this.body.add(this.smashEyes),
this.eyeStatus="smash")}else"top"===this.impactType?(c=this.topLink,d=this.topRacket,f=this.topImpactAngle,g=this.parameters.racketLength/2,n=new THREE.Vector3(0,this.parameters.bodyHeight+this.parameters.racketLength/2*Math.cos(f),this.parameters.racketLength/2*Math.sin(f)),h=n.y,e=this.getTopImpactSpeed(h)):("left"===this.impactType?(c=this.leftLink,d=this.leftRacket,h=this.leftImpactHeight,g=this.leftImpactWidth,n=new THREE.Vector3(this.leftImpactWidth,this.leftImpactHeight,0)):"right"===this.impactType&&
(c=this.rightLink,d=this.rightRacket,h=this.rightImpactHeight,g=this.rightImpactWidth,n=new THREE.Vector3(-this.rightImpactWidth,this.rightImpactHeight,0)),l=this.getImpactParams(h),f=l[0],e=l[1]),b=this.predictFallingPosition(h),"smash"===this.eyeStatus&&(this.body.remove(this.smashEyes),this.body.add(this.eyes),this.eyeStatus="common");"common"===this.eyeStatus&&(l=this.body.worldToLocal(this.shuttle.localToWorld(new THREE.Vector3)),5<l.x?(this.leftEyeBall.position.x=this.side/2-.3*this.side/2,
this.rightEyeBall.position.x=this.side/2-.3*this.side/2):-5>l.x?(this.leftEyeBall.position.x=-(this.side/2-.3*this.side/2),this.rightEyeBall.position.x=-(this.side/2-.3*this.side/2)):(this.leftEyeBall.position.x=0,this.rightEyeBall.position.x=0));0!==this.impactCount&&this.shuttle.impactCount!==this.impactCount+1||"stop-ground"===this.shuttle.state||"stop-net"===this.shuttle.state||!b||0===this.limits.xMin&&0>b.x||0===this.limits.xMax&&0<b.x||b.x<this.limits.xMin+(this.limits.xMin-this.limits.xMax)*
this.limitEpsilon||b.x>this.limits.xMax+(this.limits.xMax-this.limits.xMin)*this.limitEpsilon||b.z<this.limits.zMin+(this.limits.zMin-this.limits.zMax)*this.limitEpsilon||b.z>this.limits.zMax+(this.limits.zMax-this.limits.zMin)*this.limitEpsilon?(h=f=l=0,b=new THREE.Vector3((this.limits.xMin+this.limits.xMax)/2,0,(this.limits.zMin+this.limits.zMax)/2)):(l=this.directionWorldToLocal(this.targetPosition.clone().sub(b).setY(0)),l=l.angleTo(new THREE.Vector3(0,0,1))*(0>l.x?-1:1),n=this.body.directionLocalToWorld(n.clone()),
b=b.clone().sub(n),h=this.predictFallingTime(h),n=Math.PI/e,h=0>h||h>n?0:h<n/2?THREE.Math.clamp(h*e,0,Math.PI):THREE.Math.clamp(Math.PI/2-(h-n/2)*e,0,Math.PI));c.angleB=0;c.updateMatrixWorld();this.checkIntersect(d,this.shuttle)&&this.impactClock.getDelta()>this.impactDelta&&(d=c.directionLocalToWorld(new THREE.Vector3(0,0,1)).normalize(),this.shuttle.impact(d.clone().multiplyScalar(e*g),d,this.racketAttenuation),this.impactCount=this.shuttle.impactCount,this.healthPercent*=this.healthAttenuation,
this.impactAudio&&this.camera&&(d=this.localToWorld(new THREE.Vector3(0,0,0)).sub(this.camera.localToWorld(new THREE.Vector3(0,0,0))).length(),this.impactAudio.currentTime=0,this.impactAudio.volume=1-Math.abs(d/this.camera.far),this.impactAudio.play()),this.court&&this.player&&(this.court.reset(),this.setLimits(this.court["getArea"+this.player]())),this.onAfterImpact());c.angleB=h;f-=c.angleA;d=this.linkAngularSpeed*a*(0>f?-1:1);c.angleA+=Math.abs(f)<Math.abs(d)?f:d;c=b.clone().sub(this.position);
f=c.clone().setLength(this.healthPercent/100*a*this.bodySpeed);c=c.length()<f.length()?c:f;this.position.add(c);c=l-this.body.rotation.y;a=this.bodyAngularSpeed*a*(0>c?-1:1);this.body.rotation.y+=Math.abs(c)<Math.abs(a)?c:a},checkIntersect:function(a,b){a.updateMatrixWorld();b.updateMatrixWorld();null===b.geometry.boundingSphere&&b.geometry.computeBoundingSphere();null===a.geometry.boundingBox&&a.geometry.computeBoundingBox();var c=b.localToTarget(b.geometry.boundingSphere.center.clone(),this.parent),
